...
session_start();
    if(isset($_POST['log'])){
        ...
        $pass = $_POST['pass'];
        $cn = mysqli_connect("localhost", "cj41890_45", "cj41890_45", "cj41890_45");
        $q = mysqli_query($cn, "SELECT email, pass, company_name FROM users WHERE email = '$mail'") or die("Z");
        $res = mysqli_fetch_array($q);
        $_SESSION['company_name'] = $res['company_name'];
        if(mysqli_num_rows($q)>0){
            if($pass == password_verify($pass, $res['pass'])){
